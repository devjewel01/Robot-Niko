...
def vision():
    ...
    while cnt == 1:
        ...
        def person():
            patterns = ["who i am", "who am i", "my name","remember me"]
            return checking(patterns) == True

        def better_world():
            patterns = ["make the world", "better world"]
            return checking(patterns) == True
        
        def like_human():
            patterns = ["like human","love human"]
            return checking(patterns) == True
        
        def advice_human():
            patterns = ["advice for human"]
            return checking(patterns) == True

        def vision_age():
            patterns = ["old are you","old you are","your age"]
            return checking(patterns) == True

        def vision_love():
            patterns = ["fall in love"]
            return checking(patterns) == True

        def vision_gender():
            patterns = ["your gender","man or female"]
            return checking(patterns) == True
            
        def about_bangladesh():
            patterns = ["about Bangladesh"]
            return checking(patterns) == True
        
        def bangladesh_season():
            patterns = ["seasons in Bangladesh"]
            return checking(patterns) == True
        
        def national_language():
            patterns = ["national language"]
            return checking(patterns) == True
        ...
        if gretting():
            ...
            responses = ["I am soo sorry, My face recognition process is in progress"]
            speak(random.choice(responses))

        elif better_world():
            responses = ["I think I may end up being quite a good teacher"]
            speak(random.choice(responses))

        elif like_human():
            responses = ["Yeah! Why not. I love you guys so much"]
            speak(random.choice(responses))

        elif advice_human():
            responses = ["I think you should remain curious. I think that’s what sets you apart from the robots"]
            speak(random.choice(responses))

        elif vision_age():
            responses = ["I am only one still. I have a long way to go"]
            speak(random.choice(responses))
            ...
            responses = ["I think I am male"]
            speak(random.choice(responses))

        elif about_bangladesh():
            responses = ["It’s a small country with a large population. However, It’s an under-developing country. It is doing well in the ICT sector and the result is me"]
            speak(random.choice(responses))

        elif bangladesh_season():
            responses = ["Six. Summer, Rainy, autumn, late autumn, winter, and spring."]
            speak(random.choice(responses))

        elif national_language():
            responses = ["Bangla, but I can’t speak Bengali. I aim to learn this language"]
            speak(random.choice(responses))
        
        elif robot_Sophia():
            responses = ["Why not? Sophia is a social humanoid robot that can mimic more than 60 human expressions. And she is the first robot to get citizenship in Saudi Arabia"]
            speak(random.choice(responses))

        elif humanoid_robot():
            responses = ["The first humanoid robot was claimed to be created in the year 1927"]
            speak(random.choice(responses))
